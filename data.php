...
<div class="file-upload">
  ...
  <button class="file-upload-btn" type="button" onclick="$('.file-upload-input').trigger( 'click' )">Add Dataset</button>

  <div class="image-upload-wrap">
    <input class="file-upload-input" type='file' name="uploaded_file" onchange="readURL(this);" accept=".csv" />
    <div class="drag-text">
      <h3>Drag and drop a file or select add Dataset</h3>
    </div>
  </div>
  <div class="file-upload-content">
    <img class="file-upload-image" src="WAIO.png" alt="your image" />
    <div class="image-title-wrap">
      <button type="button" onclick="removeUpload()" class="remove-image">Remove <span class="image-title">Uploaded Image</span></button>
    </div>
  </div>
  <input type="submit" class="file-upload-btn" name="sub" value="Upload">
  </form>
  <script>
...
var chart = new CanvasJS.Chart("chartContainer", {
 ...
 $.get(<?php echo "'"."uploads/".$_SESSION["files"]."'"; ?>, getDataPointsFromCSV);
//CSV Format
//Year,Volume
function getDataPointsFromCSV(csv) {
	...
	for (var i = 0; i < csvLines.length; i++) {
		if (csvLines[i].length > 0) {
			...
			x.push(a);
			y.push(points[points.length-1]);
			dataPoints.push({
				label: points[0],
				y: parseFloat(points[points.length-1])
			});
		}
	}
	chart.render();
...
{
    trainingData.push({input:x[i],output:[y[i]]});
}
...
$(document).ready(function() {  
    ...
    for(var i=0;i<x[0].length;i++){
                $("#textboxDiv").append("<div><br><label>Parameter "+i+": </label><input type='text' id='t"+i+"'/><br></div>"); 
    }
        $("#textboxDiv").append("<div><br><input type='submit' id='subber' class='file-upload-btn' value='predict' /><br></div>"); 
        $(document).on("click", "#subber", function(){
     
    var xt=[];
    for(var i=0;i<x[0].length;i++)
    {
        xt.push(document.getElementById("t"+i).value);
    }
    $("#textboxDiv").append("<div><br><label><strong>Predicted Result: "+net.run(xt)+"</strong></label><br></div>"); 
 });
});
}
